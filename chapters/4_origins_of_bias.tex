...
THERE ARE FOUR POSSIBILITIES PRESSURES: historical naming, acquistion, cognitive. pragmatic (across different timescales). 
...
\section{Study 1b: Complexity bias for large sample of languages}
...
\subsection{Methods} \hspace*{\fill} \\
...
%%%%%%%%STUDY 2b - CDI%%%%%%%%
\section{Study 2b: Complexity bias and language acquisition}
...
\subsubsection{Stimuli} Participants were presented with a subset of words from the Words and Sentences Form of the MacArthur-Bates Communicative Development Inventory \cite<CDI;>{fenson2007}. We excluded items that were onomatopoeic sounds (e.g. ``choo choo") or contained more than one word (e.g. ``rocking chair"). The final list contained 625 items. 
\subsubsection{Procedure} We used the same norming procedure as in Study 1, except that participants were given an example of a simple and complex meaning in the instructions and  rated two anchor words (``ball" and ``motherboard;" as in Study 9 in Chapter 2). Each participant rated a sample of 40 words. 

\subsection{Results and Discussion}
The mean complexity rating for the anchor words suggested that participants were using the scale as intended (ball: $M =1.33$; $SD = .93$; motherboard: $M = 5.86$; $SD = 1.66$). Each of the 525 target words  was rated by an average of 16 participants, with a mean complexity rating of 2.77 ($SD = .77$). We next analyzed how these by-item complexity norms were related to two factors: Word length (Complexity Bias; Analysis 1) and age of acquisition (Analysis 2). 

\begin{figure*}[t!]
\begin{center}
\includegraphics[scale = .5]{figs/chap4_3.pdf}
\end{center}
\caption{ Word length as a function of complexity norms for items in the CDI.}
\label{fig:study3}
\end{figure*}

\subsubsection{Analysis 1: Complexity bias in early vocabulary}
As in previous studies, there was a reliable correlation between word length and complexity norms: Words whose meanings were rated as more complex tended to be longer (Figure \ref{fig:study3}; $r=0.35, p<.001$). This controling for word frequency.


\subsubsection{Analysis 2: Complexity and age of acquistion}
yes 
comprehesion and production

\cite{braginsky2016from}
